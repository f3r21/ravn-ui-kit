...
describe('LabelModal Component', () => {
  it('renders as a dialog popover listing every label', () => {
    render(<LabelModal labels={LABELS} onAction={vi.fn()} onClose={vi.fn()} />);
    expect(screen.getByRole('dialog', { name: 'Label' })).toBeDefined();
    expect(screen.getByText('Bug')).toBeDefined();
    expect(screen.getByText('Feature')).toBeDefined();
  });

  it('calls onAction with the clicked label', async () => {
    const handleSelect = vi.fn();
    const user = userEvent.setup();
    render(<LabelModal labels={LABELS} onAction={handleSelect} onClose={vi.fn()} />);
  ...
  it('calls onClose when Escape is pressed', async () => {
    ...
    const user = userEvent.setup();
    render(<LabelModal labels={LABELS} onAction={vi.fn()} onClose={handleClose} />);
  ...
  it('calls onClose on an outside click', async () => {
    ...
    render(
      <div>
        ...
        <LabelModal labels={LABELS} onAction={vi.fn()} onClose={handleClose} />
      </div>,
    );
  ...
  it('renames the header and the popover’s accessible name from one prop', () => {
    ...
    render(<LabelModal labels={LABELS} onAction={vi.fn()} onClose={vi.fn()} label="Category" />);
